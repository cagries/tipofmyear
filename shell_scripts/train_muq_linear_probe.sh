#!/bin/sh
for fold in 00 01 02 03; do
    python scripts/train_linear_probe.py \
      --feature-cache data/processed/jtd_group_cv_16/features/muq_10s/muq_large_msd_iter_layermean_concat.pt \
      --fold-dir data/processed/jtd_group_cv_16/manifests/fold_${fold} \
      --classes-json data/processed/jtd_group_cv_16/classes.json \
      --results-dir results/final_muq_linear_10s/fold_${fold} \
      --head linear \
      --epochs 100 \
      --batch-size 64 \
      --learning-rate 1e-3 \
      --weight-decay 0.0 \
      --seed 1337 \
      --final-train \
      --device cuda \
      --wandb-project tipofmyear \
      --wandb-run-name final_muq_linear_10s_fold${fold}
  done

python scripts/summarize_results.py \
    --results-root results/final_muq_linear_10s \
    --folds fold_00 fold_01 fold_02 fold_03 \
    --output results/final_muq_linear_10s/summary.json \
    --markdown-output results/final_muq_linear_10s/summary.md





