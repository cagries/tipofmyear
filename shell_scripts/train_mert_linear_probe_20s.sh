...
for fold in 00 01 02 03; do
    python scripts/train_linear_probe.py \
      ...
      --results-dir results/final_mert_linear_20s_no_pca/fold_${fold} \
      --head linear \
      --epochs 100 \
      --batch-size 64 \
      --learning-rate 1e-3 \
      ...
      --wandb-run-name final_mert_linear_20s_fold${fold}
  done

python scripts/summarize_results.py \
    --results-root results/final_mert_linear_20s_no_pca \
    --folds fold_00 fold_01 fold_02 fold_03 \
    --output results/final_mert_linear_20s_no_pca/summary.json \
    --markdown-output results/final_mert_linear_20s_no_pca/summary.md
